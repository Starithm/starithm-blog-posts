...
Refined analysis from the SVOM/ECLAIRs team has confirmed GRB 260727A as a long gamma-ray burst with a complex temporal structure characterized by multiple peaks and a **T90 duration of 66.5 seconds**. The event's detection across an unprecedented network of instruments—including SVOM/GRM, AstroSat/CZTI, Glowbug, and Konus-Wind—provides robust multi-wavelength constraints on the burst properties and strengthens confidence in the source localization.

A critical development comes from EP-FXT's second follow-up observation, which has identified a *faint X-ray source* associated with the burst location. The detected source exhibits *specific spectral characteristics consistent with an afterglow*, marking the *first direct evidence of X-ray emission following the initial gamma-ray event*. This detection bridges the immediate burst phase and longer-term afterglow evolution, providing essential data for constraining the burst's energetics and progenitor environment.
...
@misc{starithm2026sb26072702,
  title     = {Bright GRB 260727A detected by SVOM with multiple peaks and X-ray afterglow.},
